...
class AddonLoader(object):
    ...
    def get_instance(self, addon):
        ...
        _t = self.loaded_addons.get(addon)
        _t_i = _t(_t)
        return _t_i
...
class AddonExecutor(object):
    ...
    def _validate_seq(self, e_seq, s_seq):
    ...
    def execute_with_order(self, addon, execute_order=None, stop_order=None):
        if not addon.__addon__().startswith('jaguar'):
            raise ImportError("Addon name should start with 'jagaur'")
